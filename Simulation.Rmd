...
colnames(sce_sim)<-colnames(sce)
```

## Calculate count matrix via NB distribution
```{r}
ds = rep(rowData(sce_sim)$Dispersion, each = ncol(sce_sim)) 
mu = c(t(2^(expression_matrix))) * (2^rowData(sce_sim)$logFC)
counts =  rnbinom(n=nrow(sce_sim)*ncol(sce_sim),size=ds,mu=mu)
counts = matrix(counts,nrow = nrow(sce_sim),ncol = ncol(sce_sim))
counts(sce_sim) =  counts
```

## Calculate library size
```{r}
colData(sce_sim)$library_size <- colSums(counts)
```
