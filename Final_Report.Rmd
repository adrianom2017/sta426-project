...
```{r, fig.cap=c('PCA plot of sce_prep, sce_muscat and sce_sim','UMAP plot of sce_prep, sce_muscat and sce_sim','t-SNE plot of sce_prep, sce_muscat and sce_sim'), message=FALSE, warning=FALSE}
...
## Data visualisation
We see that in both simulation frameworks the different conditions introduce substancial variance into the data as expected. We observe that in our simulation framework the two cell types in the unperturbed condition are not as well seperated as in the muscat simulation. 
```{r}
#Plot dim reductions
par(mfrow=c(3,1))
...
## DE Analysis
We applied 7 methods in total to the simulated data sets, including
...
### True vs Predicted genes for each cluster
...
g2 = upset(upset_data_mm_ni, empty.intersections = TRUE) + 
  ggtitle(title = "counts_sum_edgeR muscat", subtitle = "Neuronal_inhib")

g3 = upset(upset_data_ms_ne, empty.intersections = TRUE) +
  ggtitle(title = "counts_sum_edgeR sce_sim", subtitle = "Neuronal_excit")
g4 = upset(upset_data_ms_ni, empty.intersections = TRUE) +
  ggtitle(title = "counts_sum_edgeR sce_sim", subtitle = "Neuronal_inhib")
print(g1)
print(g2)
print(g3)
print(g4)
...
The detection of DE genes seems to be more challenging in the correlation simulation framework as suggested by the fact that less truly DE genes are detected compared to the muscat simulation framework which does not explicilty implement a correlation structure. This indicates that the correlation reduces the power of the analysis methods and may a crucial role when trying to detect DE features.
