...
### Simulation
```{r, warning=FALSE, results="hide", warnings = FALSE, message=FALSE,include=FALSE}
...
## Data visualisation
We visualise the data in form of PCA, UMAP and t-SNE plots for the original data set and both simulations. We see that in both simulation frameworks the different conditions introduce substancial variance into the data as expected. We observe that in our simulation framework the two cell types in the unperturbed condition are not as well seperated as in the muscat simulation. Interestingly the separation of the unpertrubed cell types is only possible in the muscat simulatio framework but neither observed in the original data set nor our simulation framework. 
```{r}
#Plot dim reductions
par(mfrow=c(3,1))
...
All methods share the same upset plots. We observe that Neuronal_inhibit cluster detects fewer DE genes compared to Neuronal_excit cluster. However, the number of DE genes that correlation structure finds in each cluster individually is higher compared to muscat.
