...
#methods <- list("mm_nbinom","mm_poisson", "mm_vst_DESeq")
```
...
m_order_e = order(sim_$p_adj.loc)
...
m_order_i = order(sim_$p_adj.loc)
...
All methods share the same upset plots. We observe that Neuronal_inhibit cluster detects fewer DE genes compared to Neuronal_excit cluster. However, the numbrer of DE genes that correlation structure finds in each cluster individuallly is highter compared to muscat.
...
### Ranking of genes 
Interestingly the ranking of the genes in the two methods is exactly the same (bug in code?).
```{r}
